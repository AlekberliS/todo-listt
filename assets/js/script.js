...
//! we collect new tasks to object , completed show tasks completed defualt is false when we clicked chekbox it will be true 
...
function createTodoElement(todo) {
  ...
  if (todo.completed) {
    const checkbox = li.querySelector('.isCompleted');
    checkbox.checked = true;
    li.querySelector('span').classList.add('active');
  }

  allTodoLists.prepend(li);
}
...
function saveTodosToLocalStorage() {
  const todos = document.querySelectorAll('.all-todos li');
  const todosData = Array.from(todos).map(todoElement => {
    const checkbox = todoElement.querySelector('.isCompleted');
    return {
      text: todoElement.querySelector('span').innerText,
      completed: checkbox.checked
    };
  });
  localStorage.setItem("todos", JSON.stringify(todosData.reverse()));
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  moonIcon.addEventListener('click', () => {
    ...
    allTodoItemss.style.backgroundColor = '#fff';
  });
